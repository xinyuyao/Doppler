...
def generate_cost_features(file_name):
    ...
    g = dgl.graph((torch.tensor(in_list), torch.tensor(out_list)), num_nodes=num_node)
...
def generate_gdp_features(file_name):
    g, _, _ = generate_cost_features(file_name)
...
def get_gdp_graph(file_name, num_device):
    in_list, out_list, num_node, node_info = _read_compute_graph(file_name)
    ...
    # 1 ops type
    op_types = node_info[:, -8:]

    # 2. output shape
    output_shape = _compute_output_tensor_size(dir_g, comp_cost, num_node)

    # 3. input ops ids
    # Get predecessors for each node
    predecessors = [dir_g.predecessors(i).tolist() for i in range(num_node)]

    # Determine max number of predecessors
    max_len = max(len(p) for p in predecessors)

    # Pad with -1 and make tensor
    input_op_ids = torch.full((num_node, max_len), -1, dtype=torch.long)
    for i, preds in enumerate(predecessors):
        input_op_ids[i, : len(preds)] = torch.tensor(preds)
    ...
    dir_g.edata["feat"] = torch.zeros(
        dir_g.num_edges(), 1
    )  # Placeholder for edge features
...
def convert_node_type_to_one_hot(node_type):
    if node_type == "input":
        return [1, 0, 0, 0, 0, 0, 0, 0]
    elif node_type == "formation":
        return [0, 1, 0, 0, 0, 0, 0, 0]
    elif node_type == "complexer":
        return [0, 0, 1, 0, 0, 0, 0, 0]
    elif node_type == "squeezer":
        return [0, 0, 0, 1, 0, 0, 0, 0]
    elif node_type == "fill":
        return [0, 0, 0, 0, 1, 0, 0, 0]
    elif node_type == "select":
        return [0, 0, 0, 0, 0, 1, 0, 0]
    elif node_type == "einsummable":
        return [0, 0, 0, 0, 0, 0, 1, 0]
    elif node_type == "unknown":
        return [0, 0, 0, 0, 0, 0, 0, 1]
    else:
        raise ValueError(
            f"Unknown node type: {node_type}. Supported types are: input, formation, complexer, squeezer, fill, select, einsummable, unknown."
        )
...
def _read_compute_graph(file_name):
    ...
    for i in range(num_node):
        int_feat = list(map(int, re.findall(r"\d+", data[4 + i])))
        str_match = re.search(
            r"[a-zA-Z_]+$", data[4 + i]
        )  # matches 'einsummable' or None
        node_type_feat = str_match.group(0) if str_match else None
        node_type_one_hot = (
            convert_node_type_to_one_hot(node_type_feat) if str_match else []
        )
        node_info.append(int_feat + node_type_one_hot)
