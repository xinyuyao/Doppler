...
class Aggregator(nn.Module):
    def __init__(self,
        ...
        self.pre_layers = FNN(node_dim, [node_dim], emb_dim).to(device) # preprocess input features of the source nodes before message-passing
    ...
    def node_update(self, nodes): # update the node features based on aggregated messages 'z'
    ...
    def forward(self, g):
        if self.reverse:
            g = dgl.reverse(g)
        g.update_all(self.msg_func, fn.sum('m', 'z'), self.node_update) # fn.sum('m', 'z'): aggregates the messages 'm' from all neighbors into a single value 'z'
        h = g.ndata.pop('h')
        return h
...
class MP(nn.Module):
    def __init__(self, emb_dim,  k, device):
        ...
        self.fpa = Aggregator(emb_dim, emb_dim, emb_dim, False, device).to(device) # forward message passing
        self.bpa = Aggregator(emb_dim, emb_dim, emb_dim, True, device).to(device) # backward message passing

        self.node_transform = FNN(emb_dim, [emb_dim], emb_dim).to(device) # a multi-layer feedforward neural network
    ...
    def forward(self, g):
        ...
        def message_pass(agg, sink):
            ...
            for i in range(self.k):
                ...
                h[sink, :] = 0 # set features of sink node to zero to exclude from further update
                h = self_trans + h # current features + update features
                g.ndata['y'] = h # update "y" which stores transformed data
            return g.ndata['y']

        out_fpa = message_pass(self.fpa, len(g.nodes())-1) # might change to list of nodes for sink nodes here
        out_bpa = message_pass(self.bpa, 0)# might change to list of nodes for sink nodes here
...
class PlaceToEmbedding(nn.Module):
    def __init__(self, emb_size, k, device):
        ...
        self.agg_p = Aggregator(emb_size * 2, emb_size * 2, emb_size * 2, False, device).to(device) # mean and standard deviation
...
class PlacetoAgent:
    def __init__(self,
        ...
        self.device_policy = Device_SoftmaxActor(node_dim * 8, n_device, device, hidden_dim).to(device)
    ...
    def device_selection(self,
        ...
        if len(self.op_parents[op]):
            p_g = dgl.node_subgraph(g, [op] + self.op_parents[op]).to(self.device) # extract subgraph containing target op + parents of target op
            pred_embeddings = self.embedding.agg_p(p_g)[0, :] # extracts the embedding of the target op
        else:
            pred_embeddings = torch.zeros(self.node_dim * 2).to(self.device) # zeros holder for mean and std embedding
        ...
        # Parallels
        r = self.op_parallel[op] # retreive ops that are parallel to the current operation
        if len(r):
            r_g = dgl.graph(([0] * len(r), range(1, len(r)+1))).to(self.device) # create a custom graph with target op connect to all parallel ops
            idx = [op]+r # list of parallel op + target op index
            r_g.ndata['y'] = emb[idx, :] # assign embeddings from original graph to custom graph
            parallel_embeddings = self.embedding.agg_r(r_g)[0, :] # do aggregatioin on custom graph and extract target op embeddings
        else:
        ...
        self.entropy.append(torch.sum(F.softmax(probs, dim=0) * F.log_softmax(probs, dim=-1)))
    ...
    def finish_episode(self, rewards, dones, use_wandb, pg_weight=None, imitation_weight=None, entropy_weight=None, update_node=False, update_device=True, gamma=0.99): # use_baseline: use baseline in reward function
